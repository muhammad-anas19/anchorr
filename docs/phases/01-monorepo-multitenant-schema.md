...
This is the real output of running both quizzes back to back: five topics recur, are weak, and matter far beyond this project. Ranked by leverage. Each one leads with the actual name to search for, so this list doubles as a study list.

### 1. Root cause analysis / systematic debugging methodology
**Search:** "root cause analysis debugging", "scientific method debugging", "how to debug systematically"
**Exposed by:** C6. This is the single highest-leverage gap here. "Maybe we didn't restart docker compose" was a guess with no supporting evidence — nothing about the symptom ("password authentication failed") pointed at "needs a restart." The actual investigation had answers available the whole time: `pg_hba.conf` showed `trust` for loopback (meaning the "confirmation" test proved nothing), and `Get-NetTCPConnection -LocalPort 5432` showed two processes bound to the port. The skill isn't Postgres-specific — it's: *before proposing a fix, ask what observation would distinguish your theory from the alternatives, then go get that observation.* This is the difference between an engineer who fixes things and one who fixes things *and knows why they were broken.*

### 2. Database constraints (UNIQUE, FOREIGN KEY, NOT NULL) as invariant enforcement
**Search:** "database constraints vs application validation", "defense in depth database design", "why enforce invariants at the database layer"
**Exposed by:** C1, C2. A unique constraint, a foreign key, a `NOT NULL` — these exist precisely because application code has bugs, gets refactored by people who don't know every rule, and gets bypassed by scripts, admin tools, and future-you at 2am. The instinct to ask "where does the safety *actually* come from" — the database, not a check in a service method — is a clear tell of a backend engineer who's been burned in production versus one who hasn't yet. C2 is the sharper version of the same idea: an unimplemented safeguard isn't neutral, it's an open door.

### 3. Composite (multi-column) indexes and the leftmost-prefix rule
**Search:** "composite index leftmost prefix rule", "multi-column index order SQL", "postgres index column order"
**Exposed by:** C4. "Add an index" is a beginner's mental model; "which columns, in which order, and which query patterns does that order actually serve" is the professional one. A composite index on `(a, b)` serves `WHERE a = ?` and `WHERE a = ? AND b = ?`, but not `WHERE b = ?` alone. This shows up in every relational database, in virtually every backend codebase with more than one query pattern per table, and is a near-universal interview question for a reason.

### 4. NestJS Dependency Injection / Inversion of Control
**Search:** "NestJS dependency injection", "inversion of control container", "dependency injection vs new keyword"
**Exposed by:** C8, and originally Q9 in the diagnostic. Worth a third pass since it's stayed UNKNOWN through two teachings. The pattern (NestJS, Spring, Angular, .NET all work this way) is always the same: code declares *what it needs*, a container decides *what instance to provide*, and that indirection is what makes testing, swapping implementations, and managing object lifecycles possible without rewriting the code that depends on them. This is closer to "how virtually all serious backend frameworks are built" than a NestJS quirk.

### 5. API design: ID enumeration risk and public vs internal identifiers
**Search:** "insecure direct object reference IDOR", "sequential ID enumeration vulnerability", "public id vs internal id API design"
**Exposed by:** C10. Auto-increment IDs were a reasonable choice when nothing outside the team ever saw one. The moment an ID crosses a trust boundary (a public API, a customer-facing URL, a third-party integration), the tradeoffs that made the original decision fine no longer automatically hold. Noticing *when a decision's original context has expired* is a skill distinct from making the decision correctly the first time.
