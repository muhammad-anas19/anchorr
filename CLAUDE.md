...
8. **"Topics to master" — after BOTH quizzes (opening diagnostic and closing quiz) are scored, write a ranked list into the phase doc** of what's still weak, explicitly reframed around **transferable software engineering skill** — not Anchor trivia. Each entry should name the general principle (e.g. "composite index leftmost-prefix rule," not "Membership's indexes"), explain why it's high-leverage across most backend projects (not just this one), and link back to which quiz question exposed the gap. The bar: topics a genuinely excellent engineer has cold, not just Anchor-specific facts. This was requested explicitly by the user (2026-09-11) as a standing requirement for every phase, not a one-off — see `docs/phases/01-*.md`'s "Topics to master" section for the expected depth and framing.
